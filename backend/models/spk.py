...
class SPKWarehouseStageLog(db.Model):
    ...
    auto-inserted by the existing shift-input and packing-list-completion
    flows (see routes/production.py::create_work_order_production_record
    and routes/packing_list.py's PackingListNew completion handler).
    """
    ...
    source_id = db.Column(db.Integer, nullable=True)  # ProductionRecord.id or PackingListNew.id
