...
const RoleTable: React.FC<RoleTableProps> = ({ roles, onEdit, onDelete }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {roles.map((role) => (
        <div key={role.id} className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
          <div className="p-6">
            {/* Header */}
              ...
              {role.permissions && role.permissions.length > 0 ? (
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {role.permissions.slice(0, 5).map((permission) => (
                    <div key={permission.id} className="flex items-center text-xs text-gray-600 dark:text-gray-300">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 italic">No permissions assigned</p>
              )}
            </div>
