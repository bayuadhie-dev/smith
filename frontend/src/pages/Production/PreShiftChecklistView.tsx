...
const PreShiftChecklistView = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 print:p-2">
      {/* Compact Header */}
      <div className="flex items-center justify-between mb-4 print:mb-2">
        <div className="flex items-center space-x-3">
          <button 
            onClick={() => navigate('/app/production/pre-shift-checklist')}
            className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-800 rounded-lg print:hidden"
          >
            <ArrowLeftIcon className="w-5 h-5 text-gray-600 dark:text-gray-300" />
          </button>
          <ClipboardDocumentCheckIcon className="w-6 h-6 text-blue-600" />
          <h1 className="text-lg font-bold text-gray-900 dark:text-white">Pre-Shift Checklist</h1>
        </div>
        <div className="flex items-center space-x-2 print:hidden">
          <button onClick={() => window.print()} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-800 rounded-lg" title="Print">
      ...
      <div className={`rounded-lg p-3 mb-4 print:mb-2 border ${shiftConfig.bg} ${shiftConfig.border}`}>
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
          <div className="flex items-center space-x-2">
            <CogIcon className="w-4 h-4 text-gray-600 dark:text-gray-300" />
      ...
      {/* Stats Row */}
      <div className="flex items-center justify-center gap-6 mb-4 print:mb-2 py-2 bg-gray-50 dark:bg-gray-900 rounded-lg">
        <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold text-gray-600 dark:text-gray-300">{stats.na}</span>
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2">
                {ngItems.map((item: any, i: number) => {
                  ...
                  return (
                    <div key={i} className="bg-white dark:bg-gray-800 rounded-lg border border-red-200 p-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2 flex-1">
                          ...
                          {item.catatan && (
                            <span className="text-gray-500 dark:text-gray-400 text-xs">({item.catatan})</span>
                          )}
                          {priorityBadge && (
                        ...
                        <div className="mt-2 p-2 bg-amber-50 rounded border border-amber-200">
                          <div className="flex items-start space-x-2">
                            ...
                            <div className="text-xs">
                              ...
                              {ca.supervisor_name && (
                                <span className="text-gray-500 dark:text-gray-400 ml-1">- {ca.supervisor_name}</span>
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      {isEditing && (
                        <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-900 rounded border">
                          <div className="space-y-2">
                            <div>
                              <label className="text-xs font-medium text-gray-700 dark:text-gray-200">Prioritas:</label>
                            ...
                            <div>
                              <label className="text-xs font-medium text-gray-700 dark:text-gray-200">Keterangan untuk Maintenance:</label>
                              <textarea
                            ...
                            <div className="flex justify-end space-x-2">
                              <button
                                ...
                                className="text-xs px-3 py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded hover:bg-gray-300"
                              >
                                Batal
                              </button>
                              <button
                  ...
                  {ngItems.filter((item: any) => item.corrective_action?.repair_notes).map((item: any, i: number) => (
                    <p key={i} className="text-xs text-gray-700 dark:text-gray-200">
                      ...
                      {item.corrective_action.handled_by_name && (
                        <span className="text-gray-500 dark:text-gray-400"> - oleh {item.corrective_action.handled_by_name}</span>
                      )}
                    </p>
                  ))}
                </div>
      ...
      {Object.entries(groupedAnswers).map(([category, answers]) => (
        <div key={category} className="mb-3 print:mb-2">
          <div className="flex items-center space-x-2 mb-1">
            ...
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100">{getCategoryLabel(category)}</h3>
            <span className="text-xs text-gray-500 dark:text-gray-400">({answers.length} item)</span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-shrink-0 ml-2">
                    ...
                    {answer.status === 'NA' && (
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-gray-50 dark:bg-gray-9000 text-white text-xs font-bold">N/A</span>
                    )}
                  </div>
                </div>
              )
      ...
      {/* Footer - Print Info */}
      <div className="mt-4 pt-2 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-400 print:block hidden">
